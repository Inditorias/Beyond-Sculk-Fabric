...
public class ResonantPortalFrameTester extends PortalFrameTester {
    public ResonantPortalFrameTester() {
    ...
    public Optional<PortalFrameTester> getNewPortal(WorldAccess worldAccess, BlockPos blockPos, Direction.Axis axis, Block... foundations) {
        return getOrEmpty(worldAccess, blockPos, (customAreaHelper) -> {
            ResonantPortalFrameTester resonantAreaHelper = (ResonantPortalFrameTester) customAreaHelper;
            return resonantAreaHelper.isValidFrame() && resonantAreaHelper.foundPortalBlocks == 0;
        }, axis, foundations);
    }
    ...
    public Optional<PortalFrameTester> getOrEmpty(WorldAccess worldAccess, BlockPos blockPos, Predicate<PortalFrameTester> predicate, Direction.Axis axis, Block... foundations) {
        Optional<PortalFrameTester> optional = Optional.of(new ResonantPortalFrameTester().init(worldAccess, blockPos, axis, foundations)).filter(predicate);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis axis2 = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new ResonantPortalFrameTester().init(worldAccess, blockPos, axis2, foundations)).filter(predicate);
        }
    }
    ...
    protected boolean isEmptySpace(BlockState blockState) {
        return blockState.isIn(RegTags.Blocks.RESONANT_PORTAL_REPLACEABLE);
    }
